...
This repo is built with pnpm workspaces and uses publish tooling support from Lerna. npm and Yarn are not supported.
...
To build the editor in development, use `pnpm tinymce-grunt`. This will output to the `modules/tinymce/js` folder (`build` is effectively `dev` followed by `tinymce-grunt`).

Task names can be included, for example `pnpm tinymce-grunt bundle` will execute the bundle task. More information on TinyMCE grunt tasks is available in the [TinyMCE readme](modules/tinymce/README.md).
...
`pnpm run eslint`
runs `eslint` across the entire repository with the rule set that is required to pass in CI. Use `pnpm run eslint --fix` to automatically fix minor problems. The [ESLint vscode plugin](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) can be used to apply lint fixes on save.
...
If you are working in a single module and don't want to deal with the overheads of whole-monorepo compilation, you can use `pnpm --filter <package-name>` to run commands for a specific module. For example, `pnpm --filter @ephox/katamari test` will run tests only for the katamari module.
...
To run a single test: `pnpm run bedrock -f file`

To run a whole folder of tests: `pnpm run bedrock -d folder`
